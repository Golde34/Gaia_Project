...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model: int, seq_len: int, dropout: float):
        ...
        # Creata a vector of shape (Seq_len, 1)
...
class LayerNormalization(nn.Module):
    ...
    def __init__(self, eps: float = 1e-6) -> None: # 10**-6
        super().__init__()
        self.eps = eps
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        query = query.view(batch_size, query.shape[1], self.h, self.d_k).transpose(1, 2)
        key = key.view(batch_size, key.shape[1], self.h, self.d_k).transpose(1, 2)
        value = value.view(batch_size, value.shape[1], self.h, self.d_k).transpose(1, 2)
