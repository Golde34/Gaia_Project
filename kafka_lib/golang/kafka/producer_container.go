...
func ProducerHandleMessage(topic string) {
	...
	if producerEnv.Verbose {
		sarama.Logger = log.New(os.Stdout, "[sarama] ", log.LstdFlags)
	}

	version, err := sarama.ParseKafkaVersion(producerEnv.Version)
	if err != nil {
		log.Panicf("Error parsing Kafka version: %v", err)
	}

	producerProvider := newProducerProvider(strings.Split(producerEnv.BootstrapServers, ","), func() *sarama.Config {
	...
	for i := 0; i < producerEnv.Producers; i++ {
		wg.Add(1)
		go func() {
			defer wg.Done()
