package vn.com.viettel.vds.adapter.producer;
...
import vn.com.viettel.vds.configuration.producer.KafkaProducerRegistry;
...
public class KafkaPublisher {
    ...
    private <T> void sendMessageAsync(T data, String topic, String kafkaServer,
        ...
        ListenableFuture<SendResult<String, String>> future = this.kafkaTemplate.getKafkaTemplate(kafkaServer).send(topic, message);
        if (callback != null) {
    ...
    private <T> boolean sendMessageSync(T data, String topic, Map<String, byte[]> headers, String kafkaServer,
        ...
        ListenableFuture<SendResult<String, String>> future = this.kafkaTemplate.getKafkaTemplate(kafkaServer).send(producerRecord);
        future.addCallback(new ListenableFutureCallback<SendResult<String, String>>() {
            public void onSuccess(SendResult<String, String> result) {
                log.info("===> Sent message=[" + message + "] with offset=[" + result.getRecordMetadata().offset()
                        + "] to topic: " + topic + " SUCCESS !!!");
